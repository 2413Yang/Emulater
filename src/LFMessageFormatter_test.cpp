#include "LFMessageFormatter.cpp"

#include <gtest/gtest.h>

TEST(LFMessageFormatterTest, FormatFUNCTION) {
  uint32_t PacketIdentifier = 0x55aa1103;
  std::vector<uint8_t> buffer(3096, 'a');
  auto dataFormat = createLFMessageFormatter(std::move(buffer));
  
  uint8_t data_one[3204] = {0};
  uint8_t data_two[3204] = {0};
...
TEST(LFMessageFormatterTest, CreateLFMessageFormatterFUNCTION) {
  std::vector<uint8_t> buffer(3000, 'a'); //3070
  auto dataFormat = createLFMessageFormatter(std::move(buffer));
